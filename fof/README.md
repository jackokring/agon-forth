...
Brad Rodriguez.
Modified by S. Jackson specifically FOF.
...
```
Agon *fof, GPL3
(C) 2023 @>S. Jackson, L.C. Benschop, Brad Rodriguez
```
...
You can load a source file by typing the following comand:

`FLOAD squares.4th`

You can now view and/or edit the loaded source file with the ED
command. Press the ESC key to leave the editor. This only works if fof
was loaded low by *load.

You can go to the MOS command prompt using the `BYE` command or by pressing Ctrl-Alt-Del.

From there you can restart FOF simply by typing the run command again.
...
messages are shown. This is normal.  extend80.4th will also load the assembler
and save the rebuilt fof.bin file and exit FOF.
